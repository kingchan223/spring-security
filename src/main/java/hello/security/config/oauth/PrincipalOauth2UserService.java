...
import hello.security.config.auth.provider.NaverUserInfo;
...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        if(userRequest.getClientRegistration().getRegistrationId().equals("google")) {
            ...
            oAuth2UserInfo = new NaverUserInfo((Map)oauth2User.getAttributes().get("response"));/*네이버만 이렇게 넣어준다.*/
        ...
        if(finduser.isPresent()){
            System.out.println("OAuth 로그인을 이미 한적이 있어 자동 회원가입이 되어 있습니다.");
            User olduser = finduser.get();
            return new PrincipalDetails(olduser);
        }else {
            System.out.println("OAuth 로그인을 최초로 하셨습니다.");
